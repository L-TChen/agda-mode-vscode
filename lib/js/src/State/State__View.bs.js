...
function Impl(Editor) {
  ...
  var sendRequest = function (request, callback, state) {
    return $$Promise.flatMap(Curry._2(State.sendRequestToView, state, request), (function (response) {
  ...
  var query = function (header, placeholder, value, callbackOnQuerySuccess, state) {
    Curry._2(Editor.setContext, "agdaModeQuerying", true);
    Curry._1(Editor.View.focus, state.view);
    console.log("focus in");
    return sendRequest(/* Query */Caml_chrome_debugger.simpleVariant("Query", [
                  header,
                  placeholder,
                  value
                ]), (function (response) {
                  ...
                  if (typeof response === "number") {
                    if (response !== 0) {
                      ...
                      tmp = Curry._1(display$prime(partial_arg, undefined), state);
                    } else {
                      tmp = $$Promise.resolved(state);
                    }
                  } else {
                    tmp = Curry._1(callbackOnQuerySuccess, response[0]);
                  }
