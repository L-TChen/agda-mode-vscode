...
function Impl(Editor) {
  ...
  var modifyGoal = function (goal, f, state) {
    ...
    return $$Promise.flatMap(Curry._3(Goal.setContent, goal, state.editor, Curry._1(f, content)), (function (param) {
                  if (param) {
                    return $$Promise.resolved(state);
                  } else {
                    return Curry._3(ViewM.displayError, "Goal-related Error", "Failed to modify the content of goal #" + String(goal.index), state);
                  }
                }));
  };
  var removeBoundaryAndDestroyGoal = function (goal, state) {
    ...
    return $$Promise.flatMap(Curry._3(Editor.setText, state.editor, outerRange, content), (function (param) {
                  if (param) {
                    Curry._1(Goal.destroy, goal);
                    return $$Promise.resolved(state);
                  } else {
                    return Curry._3(ViewM.displayError, "Goal-related Error", "Unable to remove the boundary of goal #" + String(goal.index), state);
                  }
                }));
  };
